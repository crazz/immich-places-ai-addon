...
export function CandidateFacts({candidate}: {candidate: TReviewCandidate}): ReactElement {
 ...
 return <div className={'space-y-1 text-sm'}>
  ...
  <p>{camera?.radius != null ? `Estimated radius: ${camera.radius} m · ${camera.radiusBasis.replaceAll('_', ' ')} · uncalibrated` : 'Precision: unknown'}</p>
