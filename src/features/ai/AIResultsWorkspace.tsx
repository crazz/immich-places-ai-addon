...
function ResultsSession({owner}: {owner: string}): ReactElement {
 ...
 useEffect(() => {
  const restore = (): void => setState(resultLocation(new URL(window.location.href)));
...
owner={owner} query={state.query} onPageAction={cursor => navigate({...state, query: {...state.query, cursor}})} onOpenAction={item => {
    ...
    {state.reference && <div className={'max-h-[65dvh] space-y-3 overflow-y-auto'}>
     ...
     <ResultDetail owner={owner} reference={state.reference} />
                        </div>}
                  </div>}
  </DialogShell>
        </>;
}
export function AIResultsWorkspace(props: {owner: string}): ReactElement {return <ResultsSession key={props.owner} {...props} />;}
