...
function readCamera(value: unknown): TReviewCamera | null {
 const coordinate = readCoordinate(value);
 if (!coordinate || !isRecord(value) || !['point', 'area', 'site', 'city', 'region'].includes(String(value.granularity)) || !['visual_estimate', 'context_extent', 'source_reported', 'unknown'].includes(String(value.radius_basis)) || (value.estimated_radius_m !== null && !finiteRange(value.estimated_radius_m, 0, Number.MAX_VALUE))) {return null;}
...
export function readCandidate(value: unknown): TReviewCandidate | null {
 ...
 const camera = readCamera(value.camera_location); const direction = readDirection(value.camera_direction);
