...
export function validReview(review: TReview): boolean {
 ...
 if (candidates.size !== review.candidates.length || observations.size !== review.observations.length || sources.size !== review.sources.length) {return false;}
 ...
 if (review.candidates.some(item => item.observations.some(id => !observations.has(id)) || item.sources.some(id => !sources.has(id)))) {return false;}
