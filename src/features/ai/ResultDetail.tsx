...
export function ResultDetail({owner, reference, onDirtyChange, manualPendingIDs = [], onVerified}: {owner: string; reference: TResultReference; manualPendingIDs?: string[]; onVerified?: () => void; onDirtyChange?: (dirty: boolean) => void}): ReactElement {
 ...
 return <section aria-label={'Saved result detail'} className={'space-y-3 break-words'}>
  ...
  {data && <>
   ...
   <p>{`Review: ${data.entry.reviewState}`}</p>
   ...
   {review && data.entry.analysisId && <DraftReview manualPendingIDs={manualPendingIDs} onVerified={onVerified} key={`${owner}:${data.entry.analysisId}`} owner={owner} detail={data} review={review} hasManualConflict={manualPendingIDs.includes(data.entry.assetId)} onDirtyChange={onDirtyChange} />}
