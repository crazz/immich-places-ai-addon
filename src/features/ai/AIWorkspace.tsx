...
function WorkspaceSession({owner, input}: TProps): ReactElement {
 ...
 return <>
  ...
  <DialogShell isOpen={isOpen} onClose={() => {setIsOpen(false); navigate(null); setRerun(null); setView('launch');}} title={'AI Locate'} subtitle={'Analyze images and review durable jobs. Closing this window does not stop a job.'}>
   {isOpen && <div className={'max-h-[75dvh] space-y-4 overflow-y-auto p-4 text-sm [&_button]:cursor-pointer [&_button]:rounded [&_button]:border [&_button]:border-(--color-border) [&_button]:px-3 [&_button]:py-2 [&_button:disabled]:cursor-default [&_button:disabled]:opacity-50 [&_input:not([type=checkbox])]:rounded [&_input:not([type=checkbox])]:border [&_input:not([type=checkbox])]:border-(--color-border) [&_input:not([type=checkbox])]:p-2 [&_select]:rounded [&_select]:border [&_select]:border-(--color-border) [&_select]:p-2 [&_textarea]:rounded [&_textarea]:border [&_textarea]:border-(--color-border) [&_textarea]:p-2'}>
