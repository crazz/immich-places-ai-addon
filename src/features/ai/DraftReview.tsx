...
export function DraftReview({owner, detail, review, onDirtyChange, hasManualConflict, onVerified}: {owner: string; detail: TResultDetail; review: TReview; hasManualConflict?: boolean; onVerified?: () => void; onDirtyChange?: (dirty: boolean) => void}): ReactElement {
 ...
 return <section aria-label={'Local review draft'} className={'space-y-3 rounded border p-3'}>
  ...
  {draft ? <><p role={'status'}>{`Saved ${draft.state} · Revision ${draft.revision}`}</p><DraftEditor key={`${draft.id}:${draft.revision}:${editorVersion}`} draft={draft} candidates={review.candidates} hasManualConflict={hasManualConflict} isBusy={isBusy || shouldReconcile} onSave={save} onDirtyChange={dirtyChanged} /><DraftBaseline key={`baseline:${draft.id}:${draft.revision}`} owner={owner} entry={detail.entry} draft={draft} disabled={isBusy || isDirty || shouldReconcile} onSaved={setDraft} onUncertain={() => setNeedsReconcile(true)} /><WritePreview owner={owner} draft={draft} disabled={isBusy || isDirty || shouldReconcile} hasManualConflict={hasManualConflict} onPreview={setPreview} onCompare={() => {setNeedsReconcile(true); void compare();}} /><WriteOperation onVerified={onVerified} owner={owner} draft={draft} preview={preview} disabled={isBusy || isDirty || shouldReconcile || hasManualConflict} /></> : <>
