...
func (s *aiWriteStore) runOne(ctx context.Context, owner, id string) error {
	...
	defer cancel()
	if op.Plan.Version == "stack-preview-v3" || op.Plan.Version == "mirror-preview-v4" {
		return s.runStackTarget(ctx, op)
	}
	a := &aiWriteAttempt{store: s}
	if op.Status == "queued" && !s.permitsPlan(op.Plan) {
...
func (a *aiWriteAttempt) Reserve(ctx context.Context, op writeback.Operation, noop bool) error {
	...
	return s.drafts.write(ctx, func(ctx context.Context, tx *sql.Tx) error {
		...
		if _, err = tx.ExecContext(ctx, aiWriteStatement(op.Plan, `UPDATE ai_write_targets SET attempts=?,generation=generation+1,leaseToken=?,leaseUntil=?,completionKnown=?,senderActive=?,noop=?,dueAt=? WHERE userID=? AND installationID=? AND operationID=?`), attempts, a.token, s.drafts.results.jobs.now().Add(45*time.Second).UnixNano(), noop, !noop, noop, s.drafts.results.jobs.now().Add(time.Second).UnixNano(), op.Plan.Owner, op.Plan.Installation, op.ID); err != nil {
			return drafts.ErrStorage
		}
		if _, err = tx.ExecContext(ctx, aiWriteStatement(op.Plan, `UPDATE ai_write_operations SET status=? WHERE userID=? AND installationID=? AND id=?`), state, op.Plan.Owner, op.Plan.Installation, op.ID); err != nil {
			return drafts.ErrStorage
		}
...
func (s *aiWriteStore) dispatchAuthority(ctx context.Context, tx *sql.Tx, op writeback.Operation, authority aiImageAuthority) error {
	...
	var hash string
	if tx.QueryRowContext(ctx, aiWriteStatement(op.Plan, `SELECT credentialHash FROM ai_write_operations WHERE userID=? AND installationID=? AND id=?`), op.Plan.Owner, op.Plan.Installation, op.ID).Scan(&hash) != nil || hash != aiWriteCredentialHash(authority.key) {
...
func (a *aiWriteAttempt) Send(ctx context.Context, op writeback.Operation) writeback.Completion {
	err := a.store.drafts.write(ctx, func(ctx context.Context, tx *sql.Tx) error {
		...
		if tx.QueryRowContext(ctx, aiWriteStatement(op.Plan, `SELECT leaseUntil FROM ai_write_targets WHERE userID=? AND installationID=? AND operationID=?`), op.Plan.Owner, op.Plan.Installation, op.ID).Scan(&leaseUntil) != nil || leaseUntil <= a.store.drafts.results.jobs.now().UnixNano() {
	...
	var result immichwrite.Outcome
	if op.Plan.Version == "standard-preview-v2" {
		result = transport.SendStandard(ctx, a.authority.key, op.Plan)
	} else {
		result = transport.Send(ctx, a.authority.key, op.Plan.TargetID, op.Plan.Intended)
	}
...
func (a *aiWriteAttempt) fence(ctx context.Context, tx *sql.Tx, op writeback.Operation) error {
	var token string
	if tx.QueryRowContext(ctx, aiWriteStatement(op.Plan, `SELECT leaseToken FROM ai_write_targets WHERE userID=? AND installationID=? AND operationID=?`), op.Plan.Owner, op.Plan.Installation, op.ID).Scan(&token) != nil || token != a.token {
		return writeback.Failure("STALE_WORKER")
	}
	return nil
}
...
func (a *aiWriteAttempt) Sent(ctx context.Context, op writeback.Operation, outcome writeback.Completion) error {
	err := a.store.drafts.write(ctx, func(ctx context.Context, tx *sql.Tx) error {
		...
		if _, err := tx.ExecContext(ctx, aiWriteStatement(op.Plan, `UPDATE ai_write_targets SET completionKnown=?,senderActive=0,dueAt=? WHERE userID=? AND installationID=? AND operationID=?`), outcome.Known, a.store.drafts.results.jobs.now().Add(time.Second).UnixNano(), op.Plan.Owner, op.Plan.Installation, op.ID); err != nil {
			return drafts.ErrStorage
		}
		if _, err := tx.ExecContext(ctx, aiWriteStatement(op.Plan, `UPDATE ai_write_operations SET status='verifying',code=? WHERE userID=? AND installationID=? AND id=?`), outcome.Code, op.Plan.Owner, op.Plan.Installation, op.ID); err != nil {
			return drafts.ErrStorage
		}
