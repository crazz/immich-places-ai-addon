...
func TestAIProductionRechecksConsentAndPolicyAfterResolution(t *testing.T) {
	for _, kind := range []string{"consent", "policy", "revision", "cancel"} {
		t.Run(kind, func(t *testing.T) {
			...
			if state != expected || calls != 0 || f.hits.Load() != 0 {
				t.Fatal("post-resolution authority bypass", state, calls, f.hits.Load())
			}
		})
	}
}
