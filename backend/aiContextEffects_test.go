...
func TestAIContextConcurrentAttemptsNeverWriteDatabaseOrImmich(t *testing.T) {
	for _, outcome := range []string{"success", "failure", "canceled", "stale-capability"} {
		t.Run(outcome, func(t *testing.T) {
			f := newAIVisualFixture(t)
			preparer, contextReq := preparedContextAttempt(t, f)
			...
			for range 8 {
				...
				go func() {
					defer wg.Done()
					result, err := f.analyzer.analyzeContext(ctx, f.request, preparer, contextReq)
					if outcome == "success" {
