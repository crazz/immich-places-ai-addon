...
Publication permits ten active previews per current draft revision and deletes at most 100 expired, unprotected rows per pass. Stale revisions are not active authority. CH19 atomically protects a consumed preview with `protected=1` when creating its durable confirmation/audit, and preserves that reference during cleanup. The flag is only a retention guard; it grants no approval.

CH19 reloads the private stored plan, verifies exact ID/digest/scope/revision/validity, durably consumes it once, and repeats source/GPS/authority checks before dispatch. The narrow preview workflow exposes only draft reading, metadata reading and plan publication. The preview interfaces permit no mutation. A fresh read can become obsolete immediately afterward; stored `usable` status is not a remote permission lock.
...
Keep `AI_WRITE_ENABLED=false` until separately authorized disposable-photo GATE-02 evidence verifies the actual Immich version, mutation route, permissions, exact field preservation and readback. Synthetic preview tests do not close that gate. No live provider, private photo or live Immich mutation was used. See the [CH18 verification record](../openspec/changes/preview-exact-ai-write-plans/implementation-verification.md) for P01–P18 and actual checks.
