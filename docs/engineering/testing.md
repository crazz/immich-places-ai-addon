...
| `bun run test:smoke --base HEAD` | Build both applications, then run all nineteen browser smoke journeys |
...
The shared smoke gate runs two isolated installations sequentially: three legacy journeys (authentication/browsing, manual placement and GPX) with AI disabled, then private provider management, an explicit synthetic capability test, protected selection/job APIs, real Visual/Context batch execution and keyboard mixed cancellation with AI enabled. Playwright runs Chromium with one worker and no retries against production Next.js standalone output, a compiled Go backend, normal migrations and a fresh temporary SQLite directory per installation. Each test owns its synthetic account/key. Loopback ports 3080 (frontend), 8089 (backend), 8090 (fake Immich) and 8091 (test-only backend policy/restart control) must be free; existing servers are never reused. Startup is bounded to 30 seconds per server, tests to 30 seconds each (60 seconds for capability/keyboard journeys and 90 seconds for the complete batch workflow) and each installation's suite to three minutes. Graceful shutdown has an eight-second fallback; normal teardown removes temporary data.
...
CH12 adds authenticated production job APIs, real composed Visual execution, attested token/cost accounting and bounded startup/shutdown recovery. The browser suite gains a protected job API journey through the built application and proxy. The [production verification record](ai-production-jobs-verification.md) maps all 34 scenarios and distinguishes synthetic integration from deployment attestation and reference-NAS measurements.

CH13 adds real Visual/Context launch, lost-acknowledgement reconciliation, reload, execution-disabled reads and keyboard mixed cancellation to the browser suite. Frozen evidence/restart, post-resolution authority, rerun membership and DTO/polling/consent scenarios run in Go and frontend suites. See the [batch verification record](ai-batch-workflow-verification.md) for all 24 scenarios and synthetic fixture limits.
...
CH18 adds canonical preview persistence, nullable GPS/source conflicts, bounded reads, deadline/cancellation, publication races, expiry/capacity/protected-history and migration 026→027 tests. Typed component/API tests fence stale private comparisons. One built journey covers narrow-screen keyboard comparison, GET-only reload and explicit conflict recovery; the suite now contains 3 legacy and 15 AI journeys. See [preview verification](../../openspec/changes/preview-exact-ai-write-plans/implementation-verification.md) for P01–P18 and executed results. Synthetic fixtures do not verify live write compatibility.

CH19 adds real SQLite durable approval/attempt/target exclusion, exact local-socket mutation/readback, generation/owner/sync/deletion/installation races and migration 025/026/027→028 coverage. Typed component tests and one built journey cover lost confirmation acknowledgement, HTTP-origin keyboard confirmation, verified Missing GPS refresh, retained history and preserved manual edits. The suite contains 3 legacy and 16 AI journeys. See [CH19 verification](../../openspec/changes/archive/2026-09-25-execute-confirmed-ai-gps-writes/implementation-verification.md). All write fixtures are synthetic and explicit; live GATE-02 remains unverified and ordinary dispatch defaults off.

CH21 adds real SQLite exact-stack approval, independent target generations/retries, cross-version exclusion, lifecycle/publication and populated-upgrade evidence. Component and built-browser tests verify explicit three-of-five selection, partial outcomes, keyboard comparison, stale history fencing and preserved manual GPS. The suite now contains 3 legacy and 19 AI journeys. See [stack verification](../../openspec/changes/confirm-explicit-ai-stack-targets/implementation-verification.md); live stack compatibility remains separately gated.

CH22 extends synthetic fixtures with independently admitted exact namespace writes and standard-success/metadata-failure recovery. Real SQLite tests cover populated v1–v3 upgrades, immutable ownership, bounded generations, unknown senders, publication failure, two-owner cleanup/deletion and restart. UI/component checks preserve manual GPS and stale private-response fencing; the built metadata journey exercises exact comparison and a metadata-only retry. Final results and the separate live boundary are recorded in [metadata verification](../../openspec/changes/mirror-supported-ai-metadata/implementation-verification.md).
