...
These structural checks are independent of GitNexus and do not prove runtime authorization, absence of side effects or computed/dynamic loading behavior. Concrete rules must grow with real AI packages; Go builds remain authoritative for Go language validity and import cycles. F02 provides frontend unit/component tests and the AI coverage policy. F03 exercises the real built frontend, proxy, backend and SQLite with synthetic external fixtures for auth, manual placement and GPX. CH01 introduces pure provider validation under `backend/internal/ai/providers/`, focused integration adapters in `backend/aiProvider*.go`, private settings UI and backend AI coverage enforcement. CH02 adds bounded dispatch and the concrete transport under `backend/internal/aiadapters/providerhttp/`. CH03 adds the capability workflow under `backend/internal/ai/capabilities/` and private persisted observations. Browser verification runs legacy journeys with AI disabled and profile management plus synthetic capability testing with AI enabled. The [19 September NAS test](nas-provider-capability-2026-09-19.md) adds live synthetic compatibility evidence for one configuration; live rollout and writeback remain separately gated as described in the roadmap; CH12 implements durable Visual admission below.

CH08 adds pure raster preparation under `backend/internal/ai/images/` and read-only Immich/SQLite integration in `backend/aiImage*.go`. The prepared value is transient and has no provider or writer authority. There is no public preparation endpoint or production consumer; see the [caller contract](../ai-image-preparation.md).
...
CH12 adds explicit production consent/policy admission, bounded private job APIs and a fixed in-process consumer. `internal/ai/jobs/` owns normalized consent/configuration, execution policy and loop settings; root `aiProduction*.go` adapters compose existing selection/image/Visual/SQLite boundaries. Migration 023 preserves old history while excluding jobs without production admission. See the [production guide](../ai-production-jobs.md). The earlier CH08/09/11 descriptions above describe their original integration limits, superseded for Visual by CH12.

CH13 adds gallery preview/consent and durable progress/cancel/rerun UI under `src/features/ai/`, with a narrow public entry point from `PhotoListContainer`. Root adapters freeze bounded CH10 Context evidence before reservation and validate completion against stored sources; migration 024 retains immutable owner/job/item evidence. Parent membership is checked atomically for new reruns. See the [batch workflow](../ai-batch-workflow.md). This adds no manual pending-coordinate, draft or Immich-write authority.
...
CH16 adds pure draft conversion/validation in `backend/internal/ai/drafts`, root `aiDraft*.go` persistence/read-only-source adapters and revision-aware editing under `src/features/ai`. Migration 026 separates immutable revision snapshots from current state and expiring source observations. A read-only bridge reports manual pending IDs; AI drafts never enter legacy saving. See [local drafts](../ai-review-drafts.md). No provider or Immich mutation capability is added.

CH18 adds pure exact-plan validation/canonicalization and a read-only workflow in `backend/internal/ai/writepreview`, root `aiWritePreview*.go` adapters, migration 027 and typed numeric preview UI under `src/features/ai`. The durable five-minute plan has no confirmation or mutation interface. See [preview and CH19 handoff](../ai-write-previews.md).

CH19 adds deterministic execution/readback policy under `backend/internal/ai/writeback`, a dedicated non-retrying single-asset transport under `backend/internal/aiadapters/immichwrite`, and root `aiWrite*.go` persistence/runtime/sync adapters. Migration 028 retains immutable approvals and generation-fenced reconciliation separately from drafts. The writer is reachable only through explicit protected confirmation; analysis and previews retain read-only capabilities. A narrow verified-refresh callback updates the existing catalog without mutating manual pending state. See [confirmed GPS operations](../ai-gps-writes.md). Ordinary dispatch remains disabled pending GATE-02.

CH21 extends pure `writepreview`/`writeback` policies with a canonical exact-target manifest. Root `aiStack*.go` adapters retain independent target state and share the existing executor, bounded runtime and cross-version target guards. Migrations 036–042 preserve old approval bytes and add private v3 storage. AI UI remains under `src/features/ai/`; manual state crosses only the existing read-only pending-ID bridge. See [explicit stack targets](../ai-stack-writes.md). The stack capability is independently default-off.

CH22 adds pure namespace/export/readback policy under `writepreview`/`writeback`, bounded metadata transport in `internal/aiadapters/immichwrite`, and focused root `aiMirror*.go` persistence adapters. Version-4 approval binds one analyzed-photo metadata step after verified standard fields; it reuses target guards without granting metadata authority to old plans or stack siblings. Migrations 043–045 preserve private lineage, independent attempts/generations, deletion exclusion and combined history. [Optional metadata mirroring](../ai-metadata-mirroring.md) remains independently default-off.
