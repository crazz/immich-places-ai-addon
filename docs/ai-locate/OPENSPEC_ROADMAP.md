...
| CH11 `add-recoverable-ai-job-execution` | Durable job/item states, immutable terminal results, leases/fencing, cancellation, restart recovery, bounded attempts and idempotent submission. Verify the whole lifecycle with a deterministic fake provider before production dispatch exists. Define installation identity and data cleanup policy when persistence is introduced. | CH01, CH05, CH07 |
