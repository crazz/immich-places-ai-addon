...
| CH05 `freeze-explicit-ai-selections` | [Planning artifacts complete](../../openspec/changes/freeze-explicit-ai-selections/proposal.md), implementation pending: expiring user-bound explicit-ID snapshots, deduplication, eligibility/exclusions, frozen scope and minimal persistent installation binding. Later catalog changes cannot expand them. | CH04 |
| CH06 `freeze-all-matching-ai-selections` | [Planning artifacts complete](../../openspec/changes/freeze-all-matching-ai-selections/proposal.md), implementation pending: complete-query selection through shared eligibility, exact bounded counts and explicit over-limit rejection. Apply after CH05 is verified and synced. | CH05 |
| CH07 `validate-ai-analysis-results` | [Planning artifacts complete](../../openspec/changes/validate-ai-analysis-results/proposal.md), implementation pending: canonical and semantic validation for outcomes, references, camera/subject coordinates, radius/direction and language statuses; no write authority. The owner approved the v6.0.3 JSON Schema validator dependency. | — |
| CH08 `prepare-bounded-ai-images` | Fetch an authorized image, normalize the analysis copy, remove unnecessary metadata, enforce decode/request limits and clean temporary data. Never expose an Immich credential or private URL to a provider. | — |
| CH09 `analyze-single-images-in-visual-mode` | Internal Visual-mode invocation through the approved transport; validate the complete result contract, including direction and multilingual statuses. No neighboring context or writer access; no public queue-bypassing endpoint. | CH03, CH07, CH08 |
| CH10 `add-consented-ai-context` | Add bounded metadata-only context to the existing analysis path with explicit consent, source eligibility, capture-time status and provenance. Exclude hidden/inaccessible or disallowed AI-origin sources and unrelated frequent/home hints. | CH04, CH09 |
| CH11 `add-recoverable-ai-job-execution` | Durable job/item states, immutable terminal results, leases/fencing, cancellation, restart recovery, bounded attempts and idempotent submission. Verify the lifecycle with a deterministic fake provider before production dispatch. Reuse CH05's planned minimal installation identity, extending controlled reindex/invalidation and defining job/result cleanup without a competing instance key. | CH01, CH05, CH07 |
| CH12 `connect-visual-analysis-to-durable-jobs` | Execute Visual analysis from frozen selections through CH11, binding consent/profile revision and enforcing access checks, enablement, deadlines, call/token limits and bounded transport behavior. Add submission/cancellation access for the usable workflow. | CH09, CH11 |
| CH13 `add-ai-batch-progress-and-reruns` | Job launch/progress UI for supported modes, page/all-matching submission, cancellation, retry-failed and reanalysis as explicit new runs. Context-assisted dispatch is enabled only after CH10's contract is present; reruns do not replace reviewed results. | CH06, CH10, CH12 |
| CH14 `browse-persistent-ai-results` | Authorized, filtered history for completed/unknown/ambiguous/failed outcomes, independent of Missing GPS; preserve earlier results and show analysis state separately from review/write state. | CH12 |
| CH15 `review-ai-camera-and-subject-proposals` | Read-only map/panel presentation of separate camera/subject points, alternatives, uncertainty, nullable heading, evidence and language tabs. Include keyboard access and numeric presentation when maps fail. | CH14 |
...
| GATE-03 — Provider capability | Complete for the [recorded 19 September NAS configuration](../engineering/nas-provider-capability-2026-09-19.md): `gpt-5.6-sol` via existing `codex-proxy`, synthetic image/JSON/strict samples supported. Revalidate changed configurations. CH09 validates analysis integration later |
