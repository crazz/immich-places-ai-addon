...
| [CH19 `execute-confirmed-ai-gps-writes`](../../openspec/changes/execute-confirmed-ai-gps-writes/proposal.md) | Prepared, not implemented: first approved single-asset GPS mutation with durable audit, fresh conflict checks, exact targets, serialized overlap, no hidden retries, readback and restart/ambiguous-outcome reconciliation. Verified local refresh and these guarantees ship together. Live rollout requires GATE-02. | CH18, CH11 |
