...
CH02 [provider egress policy](../../openspec/changes/enforce-ai-provider-egress-policy/proposal.md) and CH03 [provider capability tests](../../openspec/changes/add-ai-provider-capability-tests/proposal.md) now have complete proposal, specification, design and task artifacts. Implement them in that order. They reuse the existing codex-proxy container on the uGreen NAS; model selection belongs to the addon profile. The first stronger-model candidate is Sol, with actual NAS compatibility still unverified. These are planning artifacts, not an installed transport or a passing live-provider gate.
