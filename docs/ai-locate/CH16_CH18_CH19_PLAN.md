...
Status: CH16 and CH18 implemented, verified and synchronized; their [draft](../../openspec/changes/persist-revisioned-ai-review-drafts/implementation-verification.md) and [preview](../../openspec/changes/preview-exact-ai-write-plans/implementation-verification.md) records contain actual verification status. CH19 is implemented, verified, synchronized and archived on 25 September 2026; its [write verification record](../../openspec/changes/archive/2026-09-25-execute-confirmed-ai-gps-writes/implementation-verification.md) covers the original 32 scenarios; its [recovery follow-up](../../openspec/changes/archive/2026-09-25-execute-confirmed-ai-gps-writes/recovery-verification.md) covers five added scenarios and the subsequent fixes. Ordinary dispatch remains disabled pending live GATE-02. Originally planned on 24 September 2026 against `0fb8eaf` on `codex/plan-ai-selection-and-validation`. All artifacts are English. OpenSpec Plus proposal, design, specification and task reviews were performed inline under the user's no-subagent instruction. No independent/subagent review is claimed.
...
| 3 | [CH19: confirmed GPS writes](../../openspec/changes/archive/2026-09-25-execute-confirmed-ai-gps-writes/proposal.md) | Confirm, execute and reconcile one exact GPS operation with durable audit | 9 / 37 / 19 |

Total: 22 requirements, 75 behavioral scenarios and 36 implementation tasks: all 36 tasks are complete, including CH19’s nineteen. Each change includes `proposal.md`, `design.md`, capability deltas, `tasks.md` and `verification-plan.md`. The individual task checklists and execution records track completion.

Apply in the requested order. Finish, verify, synchronize and commit each change before applying its successor in a fresh local task using the same checkout. The original batch excluded archiving, pushing and deployment; the user subsequently authorized CH19 archival on 25 September 2026. No push or deployment was performed. CH16 depends on implemented CH15. CH18 depends on CH16. CH19 depends on CH18 and implemented CH11; CH17 is not required for GPS-only writes. CH16 adds migration 026 and CH18 adds 027. CH19 adds migration 028, with fresh and 025/026/027 upgrade/reopen verification.

CH16's seven requirements and twenty scenarios are synchronized into maintained `ai-results-and-review`. CH18's six requirements and eighteen scenarios are synchronized into maintained `ai-immich-writeback`; CH19’s seven writeback requirements and two review requirements are now synchronized into both maintained capabilities. Do not apply CH19 before CH18 has been synchronized. The three deltas use unique added requirement names and preserve existing read-only inspection/preview requirements. CH16 and CH18 remain active approved change records; CH19’s synchronized deltas and verification records are archived.
