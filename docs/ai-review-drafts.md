...
CH18 now adds [exact read-only previews](ai-write-previews.md) for reviewed staged GPS revisions. CH19 adds [confirmed GPS writes](ai-gps-writes.md), disabled by default pending live compatibility. Queued or safely retryable approvals are canceled by a new revision; a reserved or possibly active request returns `WRITE_IN_PROGRESS` and preserves unsaved edits until the operation settles. No live provider/Immich compatibility is claimed by synthetic tests. See [draft implementation verification](../openspec/changes/persist-revisioned-ai-review-drafts/implementation-verification.md).
