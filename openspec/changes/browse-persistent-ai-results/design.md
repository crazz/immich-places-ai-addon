...
See [proposal.md](proposal.md), [CH12's admission metadata](../connect-visual-analysis-to-durable-jobs/design.md), the [PRD history contract](../../../docs/ai-locate/PRD.md#fr-09--keep-reviewable-result-history), and adopted [architecture](../../../docs/engineering/architecture.md), [testing](../../../docs/engineering/testing.md) and [coding standards](../../../docs/engineering/coding-standards.md). At `23c6ca6`, GitNexus finds `ReadAnalysis` with incomplete receiver callers; targeted source/test search confirms its private job-store reads. Migration 022 already separates immutable analyses from catalog rows, but exposes neither history queries nor public detail. The current read gate requires AI enabled. The existing thumbnail path checks visibility and uses the owner's upstream client. The shell's catalog readiness cannot be assumed for independent history.
